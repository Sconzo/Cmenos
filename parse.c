...
// prototipos de funcoes para ativacoes recursivas
...
TreeNode * stmt_sequence(void)
{
    ...
    while((token != ENDFILE)&&(token!=END)&&(token!=ELSE)&&(token!=UNTIL))
    {
        ...
        {
            if(t==NULL) t = p = q;
            else // funcao basica do analisador sintatico
            {
                p->sibling = q;
                p = q;
            }
...
TreeNode * expr(void)
{
    ...
    if((token==LT)||(token==EQ))
    {
        ...
        match(token);
        if(t!=NULL) t->child[1] = simple_expr();
    }
    return t;
}
...
//******************************
// Funcao basica do analisador sintatico
//*****************************

//funcao que retorna a arvore sintatica contruida
